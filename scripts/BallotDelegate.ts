...
async function main() {
  ...
  const delegateVoteTx = await ballotContract.delegate(ADDRESS);
  const delegateVoteTxReceipt = await delegateVoteTx.wait();
  console.log(`Transaction completed at block ${delegateVoteTxReceipt.blockNumber} with hash ${delegateVoteTxReceipt.blockHash}`);
